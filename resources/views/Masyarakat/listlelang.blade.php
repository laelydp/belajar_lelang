...
@if($item->pemenang == Auth::user()->name)
    <div class="alert alert-success">
        <strong>Selamat!</strong> Kamu memenangkan lelang untuk barang <a href="{{ route('lelangg.create', $item->id )}}">{{ $item->barang->nama_barang }}</a>
    </div>
@endif
@endforeach
    ...
    <div class="col-sm-3">
      <div class="card animate__animated animate__delay-2s animate__fadeInUp">
        ...
        <div class="card-body">
          ...
          <a href="{{ route('lelangg.create', $item->id)}}" class="btn btn-primary animate__animated animate__delay-3s animate__bounceIn">TAWAR SEKARANG</a>
          @else
          <p class="card-text">Harga Akhir: @currency($item->harga_akhir)</p>
          <p class="card-text">Pemenang: {{ $item->pemenang }}</p>
          <a href="{{ route('lelangg.create', $item->id)}}" class="btn btn-info animate__animated animate__delay-3s animate__bounceIn">LIHAT DETAIL</a>
          @endif
        </div>
      </div>
    </div>
